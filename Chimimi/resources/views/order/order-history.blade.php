{{-- filepath: Chimimi/resources/views/order/order-history.blade.php --}}
          ...
          @else
            <div class="d-flex flex-column gap-3">
              @foreach ($orderHistory as $history)
                @php
                  $grouped = $history->products->groupBy(function($p){ return $p->pivot->product_type; });
                  $orderTotal = $history->products->sum(fn($p) => ($p->pivot->price_at_order ?? 0) * ($p->pivot->quantity ?? 0));
                @endphp

                <div class="card shadow-sm p-3" style="border-radius:12px;border:1px solid rgba(255,111,97,0.08);background:#fff;">
                  <div class="d-flex justify-content-between align-items-start mb-2">
                    <div>
                      <div class="fw-bold" style="color:#ff6f61;font-size:1.05rem;">Order #{{ $history->id }}</div>
                      <div class="small text-muted">{{ $history->created_at->format('d M Y H:i') }}</div>
                    </div>
                    <div class="text-end">
                      <span class="badge rounded-pill px-3 py-2 fw-bold" style="{{ $history->isPaid ? 'background:#28a745;color:#fff;' : 'background:#ffe066;color:#212529;' }}font-size:0.9rem;">
                      ...
                      <li class="d-flex justify-content-between align-items-center py-2"
                        ...
                        <div style="min-width:0;">
                          <div class="d-flex align-items-center gap-2">
                            <div class="fw-bold text-truncate" style="color:#ff6f61;">{{ $item->name }}</div>
                          ...
                          <div class="small text-muted mt-1">
                            Unit: Rp. <span class="fw-bold">{{ number_format($item->pivot->price_at_order,0,',','.') }}</span>
                            &nbsp;·&nbsp;
                            Qty: <span class="fw-bold" style="color:#ff6f61;">{{ $item->pivot->quantity }}</span>
                          </div>
                        </div>
                        <div class="fw-bold">Rp. {{ number_format($item->pivot->price_at_order * $item->pivot->quantity,0,',','.') }}</div>
                      </li>
                    @endforeach
                  </ul>
                  ...
                  <div class="mt-3 d-flex justify-content-between align-items-center">
                    ...
                    <div class="fw-bold" style="color:#ff6f61;font-size:1.03rem;">Rp. {{ number_format($orderTotal,0,',','.') }}</div>
                  </div>
                  ...
                  @if (!empty($history->notes))
                    ...
                    <div class="mt-1" style="white-space:pre-wrap;color:#333;">{{ $history->notes }}</div>
                  @endif
                ...
                <div class="d-flex justify-content-center mt-4">
                    <nav>
                        <ul class="pagination" style="--bs-pagination-bg:#fffbe6;--bs-pagination-border-color:#ff6f61;gap:0.7rem;">
                            ...
                            @else
                                <li class="page-item">
                                    <a class="page-link" href="{{ $orderHistory->previousPageUrl() }}" rel="prev" style="color:#ff6f61;background:#fffbe6;border-radius:18px;border:2px solid #ff6f61;">&laquo;</a>
                                </li>
                            @endif
                            ...
                            @if ($orderHistory->hasMorePages())
                                <li class="page-item">
                                    <a class="page-link" href="{{ $orderHistory->nextPageUrl() }}" rel="next" style="color:#ff6f61;background:#fffbe6;border-radius:18px;border:2px solid #ff6f61;">&raquo;</a>
                                </li>
                            @else
                                <li class="page-item disabled">
