                ...
                <div class="card shadow-lg p-4"
                    ...
                    <form method="POST" action="{{ route('admin.products.update', $product->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" class="form-control" required style="border-radius:16px;" value="{{ old('name', $product->name) }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold" style="color:#ff6f61;">Category</label>
                            <input type="text" name="category" class="form-control" required style="border-radius:16px;" value="{{ old('category', $product->category) }}">
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" name="price_frozen" class="form-control" required min="0" step="1000" style="border-radius:16px;" value="{{ old('price_frozen', $product->price_frozen) }}">
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold" style="color:#ff6f61;">Price Cooked</label>
                            <input type="number" name="price_cooked" class="form-control" required min="0" step="1000" style="border-radius:16px;" value="{{ old('price_cooked', $product->price_cooked) }}">
                        </div>
                        <div class="mb-3">
                            ...
                            @if ($product->image_path)
                                <img src="{{ asset('storage/' . $product->image_path) }}" alt="Current Image" class="mt-2" style="max-width:120px;border-radius:12px;">
                            @endif
                        </div>
                        <div class="d-flex justify-content-center">
                            <button type="submit" class="btn px-5 py-2"
                                style="background:#ff6f61;color:#fff;font-weight:600;border-radius:24px;box-shadow:0 2px 12px rgba(255,111,97,0.10);">Update Product</button>
                        </div>
                    </form>
                </div>
            </div>
