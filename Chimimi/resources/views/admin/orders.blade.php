                            ...
                            <div class="card p-3" style="border-radius:18px;background:#fffbe6;border:2px solid #ff6f61;">
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        ...
                                        <div class="text-muted">By:
                                            {{ $order->user->username ?? ($order->user->email ?? '—') }}</div>
                                    ...
                                    <div class="text-end d-flex flex-column align-items-end" style="gap:0.5rem;">
                                        @if (!$order->isPaid)
                                            <form method="POST" action="{{ route('admin.orders.pay', $order->id) }}"
                                        ...
                                        @else
                                            <span class="badge rounded-pill px-3 py-2 fw-bold"
                                                style="background:#28a745;color:#fff;">Paid</span>
                                        @endif
                                    </div>
                                </div>
                                            ...
                                            @foreach ($order->products as $product)
                                                <li class="d-flex justify-content-between align-items-center py-2"
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="text-muted small">{{ $product->pivot->product_type }} —
                                                            Qty: {{ $product->pivot->quantity }}</div>
                                                    </div>
                                ...
                                <div class="mt-3 mb-2 d-flex justify-content-between align-items-center">
                                    ...
                                    <div class="fw-bold" style="color:#ff6f61;">
                                        Rp.
                                        {{ number_format($order->products->sum(function ($p) {return $p->pivot->price_at_order * $p->pivot->quantity;}),0,',','.') }}
                                    </div>
                                </div>
